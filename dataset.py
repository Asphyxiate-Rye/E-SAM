...
class MMWHS_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        elif self.split == "train_non_zero_label":
            slice_name = self.sample_list[idx].strip('\n')
            data_path = os.path.join(self.data_dir, slice_name+'.npz')
            data = np.load(data_path)
            image, label = data['image'], data['label']
        ...
        image = (image + 750.0) / (750.0 + 750.0)   # -150  800
        image = np.clip(image, 0.0, 1.0)

        # assert np.unique(label).tolist() in [205., 420., 500., 550., 600., 820., 850.]
        label[label == 205.] = 1
        label[label == 420.] = 2
        label[label == 500.] = 3
        label[label == 550.] = 4
...
class RandomGenerator_prompt(object):
    def __init__(self, args, output_size, num_points, low_res):
        self.output_size = output_size
        self.args = args
        self.low_res = low_res
        self.bbox_shift = 5
    ...
    def __call__(self, sample):
        ...
        gt2D = np.uint8(label == random.choice(label_ids.tolist()))
        assert np.max(gt2D) == 1 and np.min(gt2D) == 0.0, "ground truth should be 0, 1"

        image = torch.from_numpy(image.astype(np.float32)).unsqueeze(0)
        image = repeat(image, 'c h w -> (repeat c) h w', repeat=3)

        gt2D = torch.from_numpy(gt2D.astype(np.float32))

        if 'centroid' in self.args.which_model:
            random_point = get_centroid(gt2D)
        elif 'bbox' in self.args.which_model:
            random_point = get_bbox(gt2D)
        elif 'point' in self.args.which_model:
            random_point = get_points(gt2D, num_points=self.num_points)

        sample = {'image': image, 'label': gt2D.long(), 'prompt': torch.tensor(random_point).float()}
        return sample
