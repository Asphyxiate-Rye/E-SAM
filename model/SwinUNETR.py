...
def window_partition(x, window_size):
    ...
    if len(x_shape) == 5:
        ...
        windows = (
            x.permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous().reshape(-1, window_size[0] * window_size[1] * window_size[2], c)
        )
    elif len(x_shape) == 4:
        b, h, w, c = x.shape
...
def window_reverse(windows, window_size, dims):
    ...
    if len(dims) == 4:
        ...
        x = x.permute(0, 1, 4, 2, 5, 3, 6, 7).contiguous().reshape(b, d, h, w, -1)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, normalize=True):
        ...
        x1, output1 = self.layers1[0](x0.contiguous())
        # x1_out = self.proj_out(x1, normalize)

        x2, output2 = self.layers2[0](x1.contiguous())
        # x2_out = self.proj_out(x2, normalize)

        x3, output3= self.layers3[0](x2.contiguous())
        # x3_out = self.proj_out(x3, normalize)

        x4, output4 = self.layers4[0](x3.contiguous())
        # x4_out = self.proj_out(x4, normalize)
        return output1 + output2 + output3 + output4


if __name__ == '__main__':
    model = SwinUNETR(img_size=(512, 512), in_channels=1, out_channels=4, feature_size=48, drop_rate=0.0,
                       attn_drop_rate=0.0, dropout_path_rate=0.0, use_checkpoint=False, spatial_dims=2)
