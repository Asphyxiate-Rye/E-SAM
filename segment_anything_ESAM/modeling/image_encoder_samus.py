...
class ImageEncoderViT_SAMUS(nn.Module):
    def __init__(
            self,
            img_size: int = 256,
            patch_size: int = 16,
        ...
        if use_abs_pos:
            ...
            self.pos_embed = nn.Parameter(
                torch.zeros(1, img_size // patch_size, img_size // patch_size, embed_dim)  # torch.zeros(1, 1024//16, 1024//16, embed_dim)
            )
            self.post_pos_embed = PostPosEmbed(embed_dim=embed_dim, ori_feature_size=img_size // patch_size,
