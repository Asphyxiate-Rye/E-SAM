...
class SS2D(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        x_hwwh = torch.stack([x.reshape(B, -1, L), torch.transpose(x, dim0=2, dim1=3).contiguous().reshape(B, -1, L)],
        ...
        xs = xs.float().reshape(B, -1, L)  # (b, k * d, l)
        dts = dts.contiguous().float().reshape(B, -1, L)  # (b, k * d, l)
        Bs = Bs.float().reshape(B, K, -1, L)  # (b, k, d_state, l)
        Cs = Cs.float().reshape(B, K, -1, L)  # (b, k, d_state, l)
        Ds = self.Ds.float().reshape(-1)  # (k * d)
        As = -torch.exp(self.A_logs.float()).reshape(-1, self.d_state)  # (k * d, d_state)
        dt_projs_bias = self.dt_projs_bias.float().reshape(-1)  # (k * d)
...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, depths=[2, 2, 6, 2],
...
class SwinUMamba(nn.Module):
    ...
    def forward(self, x_in):
        x1 = self.stem(x_in)
        output_all = self.vssm_encoder(x1)
...
if __name__ == '__main__':
    model = SwinUMamba().to('cuda:1')
    data = torch.rand((1, 1, 224, 224)).to('cuda:1')
    output = model(data)
    print(output.shape)